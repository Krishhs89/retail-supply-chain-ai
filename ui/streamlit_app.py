...
_SESSION_QUERY_LIMIT = 30   # max AI queries per browser session
...
def _init_state():
    defaults = {
        ...
        "session_start_ts": time.time(),   # rate-limit window start
    }
...
def _check_rate_limit() -> bool:
    """Return True if the session is within the query limit, False if exceeded."""
...
with st.sidebar:
    ...
    _q_used = st.session_state.get("session_queries", 0)
    _q_pct = min(1.0, _q_used / _SESSION_QUERY_LIMIT)
    st.progress(_q_pct, text=f"Query budget: {_q_used}/{_SESSION_QUERY_LIMIT}")
    if _q_used >= _SESSION_QUERY_LIMIT:
        st.warning("Session query limit reached. Refresh the page to start a new session.")
...
def _chat_content(full_width: bool = True):
    ...
    if prompt:
        if not _check_rate_limit():
            st.error(f"Session query limit ({_SESSION_QUERY_LIMIT}) reached. Refresh the page to start a new session.")
            st.stop()
