"""
Retail Supply Chain Optimization — Streamlit UI  (v3)

Tabs (7):
  0. ⭐ Guide      — onboarding, quick start, glossary, sample queries
  1. Dashboard     — live network KPI tiles + status
  2. Chat          — streaming multi-agent conversational interface
  3. Simulate      — Price Cascade / Supply Alert / Demand Forecast (selectbox sub-nav)
  4. Scenarios     — Scenario Planner / Strategy Canvas (radio sub-nav)
  5. Operations    — Shelf & Store / Financial Impact / Data Sources (selectbox sub-nav)
  6. Architecture  — Network Graph + Workflow (expander)
"""
...
[data-testid="stSidebar"] .stButton > button:hover {
    background: #0071ce; border-color: #0071ce;
}
...
def _init_state():
    defaults = {
        ...
        "pipeline_version": "V2 — LangGraph",   # v3 default: V2
        "hist_price": [],
        "hist_supply": [],
        "hist_forecast": [],
        "hist_scenario": [],
...
Where fᵢ = factor signal, wᵢ = learned weight for each of:
...
def _render_details(tab_key: str):
    """Combined formula + data sources in a single expander at bottom of results."""
...
def _render_run_history(items: list, empty_msg: str = "No runs yet."):
    """Compact run history expander at the bottom of results."""
...
with st.sidebar:
    ...
    with st.expander("⚙️ Advanced", expanded=False):
        ...
        if "V1" in pipeline_ver:
            max_iter = st.slider(
                "Max iterations (V1 only)", min_value=3, max_value=25, value=10,
...
**This AI reasons through all of those connections at once** — in seconds — using 17 specialist tools and two AI pipelines (a direct Claude agent and a LangGraph multi-agent graph).
    ...
    with qs3:
        ...
        Go to the <strong>Architecture</strong> tab, expand the Workflow section, pick your trigger and let the AI run the full structured analysis with a Day 0→30 ripple timeline.
        </div>
        """, unsafe_allow_html=True)
    ...
    tab_overview = [
        ("📊", "Dashboard", "Network at a glance — carrier status, DC inventory KPIs, strike timeline. No API key needed."),
        ("💬", "Chat", "Core AI interface. Ask anything in plain English; the agent calls the right tools in sequence."),
        ("🔬", "Simulate", "**Price Cascade** · **Supply Alert** · **Demand Forecast** — pick from the dropdown. Full charts and KPIs."),
        ("⚖️", "Scenarios", "**Scenario Planner** for side-by-side comparison + conflict detection. **Strategy Canvas** for drag-and-drop scenario building."),
        ("🏭", "Operations", "**Shelf & Store** replenishment chain · **Financial Impact** P&L waterfall · **Data Sources** provenance tracker."),
        ("🕸", "Architecture", "**Network Graph** — live LangGraph DAG with step-through execution trace. **Workflow** builder in the expander below."),
    ]

    for icon, name, desc in tab_overview:
...
| **Replenishment Lag** | Time from order to shelf: 3–4 days + 30% chance of +3 extra |
...
| **VMI** | Vendor-Managed Inventory — stock owned by manufacturer, not retailer |
        """)
    with g2:
        st.markdown("""
| Term | Meaning |
...
| **Planogram** | Shelf layout plan. Orders exceeding shelf capacity go to back-of-store |
| **Scenario Conflict** | Two simultaneous decisions creating opposing supply pressure |
| **Confidence Interval** | Forecast uncertainty band. Widens 15% per 4-week horizon |
    ...
    fc4.metric("App Tabs", "7", "streamlined v3")
...
A graph of 12 specialist agents. A Router classifies your query, routes it to the right domain node, runs supporting nodes, then a Synthesizer combines everything into one response.

**V1 — Agentic Loop**
One Claude claude-sonnet-4-6 agent. It sees all 17 tools and decides which ones to call and in what order.

Switch in the **⚙️ Advanced** sidebar expander. Both hit the same mock data layer.
        """)
    with arch2:
        st.code("""
User Query
    │
    ▼
[PIPELINE TOGGLE — ⚙️ Advanced sidebar]
    │
    ├── V2 (default): LangGraph Multi-Agent Graph
...
with tab_dash:
    ...
    base_weekly_rev = (mock_executor.PRODUCTS["HUG48-3"]["base_price"]
                       * mock_executor.PRODUCTS["HUG48-3"]["base_demand_per_store_week"]
                       * total_stores)
            ...
            if not key_ok:
                ...
                if result:
                    ...
                    if result["tool_calls_made"]:
                        with st.expander(f"🔧 {len(result['tool_calls_made'])} tool call(s) detail", expanded=False):
                            tc_df = pd.DataFrame([{
                                "Tool": tc["tool"],
...
# ════════════════════════════════════════════════════════════════════════════
# TAB 3 — SIMULATE  (Price Cascade / Supply Alert / Demand Forecast)
# ════════════════════════════════════════════════════════════════════════════
with tab_simulate:
    ...
    if sim_choice == "Price Cascade":
        ...
        with col3:
            pc_new_price = st.number_input("New Price ($)",
                                           value=float(prod_info.get("base_price", 12.99)) + 1.50,
        ...
        _render_run_history(st.session_state.hist_price, "Run a simulation to see history.")
    ...
    # ── Supply Alert ──
        ...
        if run_sa:
            ...
            if res.get("error"):
                ...
                if crit > 0:
                    st.error(f"CRITICAL: {crit} location(s) hit stockout before replenishment arrives.")
                elif warn > 0:
        ...
        _render_run_history(st.session_state.hist_supply, "Run a disruption analysis to see history.")

    # ── Demand Forecast ──
    elif sim_choice == "Demand Forecast":
            ...
            if res.get("error"):
                ...
                with col_note:
                    ...
                    if not d["is_reliable"]:
                        st.error(f"UNRELIABLE ({acc*100:.1f}%). DO NOT pass to PO system.")
                    elif acc < 0.70:
                        st.warning(f"Below 70% threshold — use with caution.")
                    else:
        ...
        _render_run_history(st.session_state.hist_forecast, "Run a forecast to see history.")


# ════════════════════════════════════════════════════════════════════════════
# TAB 4 — SCENARIOS  (Scenario Planner / Strategy Canvas)
# ════════════════════════════════════════════════════════════════════════════
with tab_scenarios:
    scen_choice = st.radio(
        "Mode:",
        ["Scenario Planner", "Strategy Canvas"],
        horizontal=True, key="scen_choice",
    )
    st.divider()
    ...
    if scen_choice == "Scenario Planner":
        ...
        for i, (col, label, df) in enumerate(zip(cols, labels, defaults)):
            with col:
                ...
                sup_red = st.number_input("Supply cut %", value=float(df["sup_red"]),
                                          min_value=0.0, max_value=100.0, step=5.0, key=f"sc_sr{i}")
        ...
        if st.button("Compare + Detect Conflicts", type="primary", key="sc_run"):
            ...
            for conflict in conf_res.get("data", {}).get("conflicts", []):
                ...
                msg = (f"**{icon}:** '{conflict['scenario_a']}' + '{conflict['scenario_b']}' — "
        ...
        _render_run_history(st.session_state.hist_scenario, "Run a scenario comparison to see history.")
            ...
            if not blocks:
                ...
                if st.button("▶ Run Analysis", type="primary", use_container_width=True, key="cv_run"):
                    ...
                    for b in blocks:
                        ...
                        rows.append({
                            "Scenario": b["label"],
                            "Rev %": fmt_p(b["rev"]), "Rev $": fmt_d(r_d),
                            "Mgn %": fmt_p(b["mgn"]), "Mgn $": fmt_d(m_d),
                            "Demand %": fmt_p(b["dem"]), "Risk": b["risk"],
                        })
                    if penalty > 0:
                        rows.append({
                    ...
                    if conflicts:
                        for cf in conflicts:
                            st.error(f"⚡ Conflict detected: **{cf}** — resolve before activating.")
                    else:
                    ...
                    if "price-up" in ids and "prestock" in ids:
                        recs.append("✅ **Recommended sequence**: Execute Pre-Build Inventory before Price Increase.")
                    if any(x in ids for x in ["tpr", "display", "bogo"]):
                        recs.append("📦 **Align promotions** with DC replenishment cycle to prevent stockout.")
                    if any(x in ids for x in ["strike", "port-delay", "dc-closure"]):
                        recs.append("🚚 **Activate alternate carriers** in affected regions within 48h.")
...
# ════════════════════════════════════════════════════════════════════════════
# TAB 5 — OPERATIONS  (Shelf & Store / Financial Impact / Data Sources)
# ════════════════════════════════════════════════════════════════════════════
with tab_ops:
    ...
    if ops_choice == "Shelf & Store":
        ...
        _render_run_history(st.session_state.hist_shelf, "Run a replenishment analysis to see history.")
    ...
    # ── Financial Impact ──
        ...
        _render_run_history(st.session_state.hist_finance, "Run a P&L calculation to see history.")
...
# ════════════════════════════════════════════════════════════════════════════
# TAB 6 — ARCHITECTURE  (Network Graph + Workflow expander)
# ════════════════════════════════════════════════════════════════════════════
with tab_arch:
    import datetime as _dt
    ...
    st.caption(
        ...
        "Run a query in Chat, then use **Step-Through** to replay how data flowed."
    )
...
| Scenario planning | router → scenario_planning → synthesizer |
    ...
    with st.expander("🔄 Workflow Builder — Step-by-Step Scenario Analysis", expanded=False):
        st.caption(
            ...
            "then let the AI run the full cascade analysis with a Day 0→30 ripple timeline."
        )
        ...
        wf_trigger_sel = st.selectbox(
            "Select trigger:",
            list(trigger_options.keys()),
            key="wf_trigger_sel",
        )
        ...
        if trigger_key == "price_change":
            ...
            with p_col1:
                wf_current_price = st.number_input("Current Price ($)", value=float(base_price_wf), step=0.01, key="wf_cur_price")
            with p_col2:
                wf_new_price = st.number_input("New Price ($)", value=float(base_price_wf * 1.10), step=0.01, key="wf_new_price")
            ...
            with d_col1:
                wf_carrier = st.selectbox("Affected Carrier",
                    ["TruckCo A", "TruckCo B", "TruckCo C", "TruckCo D"], index=1, key="wf_carrier")
            with d_col2:
            ...
            wf_demand_driver = st.selectbox("Primary Driver",
            ...
            wf_season = st.selectbox("Seasonal Event",
            ...
            wf_competitor = st.selectbox("Competitor",
                ["Costco", "Target", "Amazon", "Kroger", "Aldi"], key="wf_competitor")
        ...
        def _build_workflow_query():
            ...
            if trigger_key == "price_change":
                pc = round((wf_new_price - wf_current_price) / wf_current_price * 100, 1)
                return (
                    f"Analyze a price change for {sku_name} ({wf_sku}) from ${wf_current_price:.2f} "
                    f"to ${wf_new_price:.2f} ({'+' if pc > 0 else ''}{pc:.1f}%) "
                    f"in the {region_str} region over a {horizon_str} horizon. "
                    f"Optimize for: {obj_str}. "
                    f"Cover: demand elasticity, inventory and replenishment impact, "
                    f"carrier capacity, financial margin with vendor trade netting, "
                    f"and scenario conflict detection. Provide 3 ranked options."
                )
            elif trigger_key == "supply_disruption":
                dur = st.session_state.get("wf_disrupt_days", 14)
                carrier = st.session_state.get("wf_carrier", "TruckCo B")
                return (
                    f"{carrier} is experiencing a supply disruption expected to last {dur} days, "
                    f"affecting {sku_name} ({wf_sku}) in the {region_str} region. "
                    f"Optimize for: {obj_str} over {horizon_str}. "
                    f"Analyze: stockout risk by DC, alternate carrier options, revenue at risk, "
                    f"and a mitigation plan with ranked options."
                )
            elif trigger_key == "demand_shift":
                ...
                return (
                    ...
                    f"Optimize for: {obj_str}. Analyze: inventory adequacy, replenishment adjustment needs, "
                ...
                return (
                    ...
                    f"demand elasticity impact, margin erosion, and vendor trade adjustments. "
                ...
                return (
                    ...
                    f"Analyze: inventory build requirements, replenishment schedule, carrier capacity. "
                ...
                return (
                    ...
                    f"Analyze: demand cannibalization, price match vs. differentiate options, margin impact, "
                    f"and 3 ranked competitive response strategies."
                )
        ...
        with run_col1:
            run_workflow = st.button("Run Analysis", type="primary", use_container_width=True, key="wf_run_btn")
        with run_col2:
            wf_pipeline = st.selectbox(
                "Pipeline", ["V2 — LangGraph", "V1 — Agentic Loop"],
                key="wf_pipeline_sel", label_visibility="collapsed",
            )
        ...
        if run_workflow:
            ...
            if wf_result:
                ...
                st.session_state.hist_workflow.append({
                    "header": f"{wf_trigger_sel}  |  {wf_sku}",
                    "meta": f"Region: {wf_region.split(' — ')[0]}  |  {wf_pipeline}",
        ...
        if wf_last:
            ...
            if tool_calls:
                with st.expander(f"Tools Used ({len(tool_calls)})", expanded=False):
                    tc_rows = [{"Tool": tc.get("tool",""), "Input": str(tc.get("input",""))[:80],
                                "Result Preview": str(tc.get("result",""))[:80]} for tc in tool_calls]
                    if tc_rows:
            ...
            domain_colors = {
                "Pricing": "#0071ce", "Demand": "#f39c12", "Inventory": "#27ae60",
                "Supply Chain": "#e74c3c", "Finance": "#8e44ad", "Strategy": "#2980b9",
            }
            ...
            for domain, grp in timeline_df.groupby("Domain"):
                ...
                if len(grp) > 1:
                    fig_timeline.add_trace(go.Scatter(
                        x=grp["Day"].tolist(), y=[domain] * len(grp),
                        mode="lines",
            ...
            fig_timeline.update_layout(
                ...
                xaxis_title="Day", yaxis_title="Domain",
                height=380, xaxis=dict(range=[-1, 32]),
